...
export default function EditJob() {
   const navigate=useNavigate()
    ...
    const formattedDate = formData.dueDate ? formData.dueDate.split('T')[0] : '';
        ...
        const fetchJobDetails = async () => {
            try {
                ...
                setFormData({
                    ...
                    openings: jobData.openings || 0,
                    location: jobData.location || [],
                    jobType: jobData.jobType || "",
                    experience: {
                        minExp: jobData.experience?.minExp || '',
                        maxExp: jobData.experience?.maxExp || ""
                    },
                    skills: jobData.skills || [],
                    dueDate: jobData.dueDate || "",
                    salary: {
                        minSalary: jobData.salary?.minSalary || "",
                        maxSalary: jobData.salary?.maxSalary || ""
                    }
                });
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`http://localhost:3456/update-job/${id}`, formData, {
                headers: {
                    Authorization: localStorage.getItem('token')
                }
            });
